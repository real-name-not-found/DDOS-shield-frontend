...
export default function AnalysisHistoryTable({ history = [], onSelectIP }) {
    ...
    const getStatusInfo = (score) => {
        if (score >= 80) return { label: 'Critical', color: 'bg-risk-critical', textColor: 'text-risk-critical' };
        if (score >= 50) return { label: 'Warning', color: 'bg-risk-warning', textColor: 'text-risk-warning' };
        if (score >= 20) return { label: 'Moderate', color: 'bg-yellow-500', textColor: 'text-yellow-600' };
        return { label: 'Clean', color: 'bg-risk-ok', textColor: 'text-risk-ok' };
    };
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left text-sm">
                            ...
                            <tbody className="divide-y divide-black/10">
                                {pageData.map((item) => {
                                    const status = getStatusInfo(item.abuseScore || 0);
                                    return (
                                        <tr
